            ...
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title">{{trans("_menu.product.car.dealer.download.modelsadd.title")}}</h3>
...
$(document).ready(function() {
    ...
    table = $('#dt_basic').DataTable({
        ...
        "aoColumns": [
            {
                ...
                "mRender":function(data,type,row){
                    return '<input type="checkbox" name="models" value="'+row.iId+'">';
                }
            },
			{"sTitle":"編號","mData":"iId", "sName": "iId"},
			{"sTitle":"車款名稱","mData":"vCarModelName", "sName": "vCarModelName"},
    		{"sTitle":"車款簡介","mData":"vSummary", "sName": "vSummary"},
        ...
        "sAjaxSource": "{{ url('web/product/car/dealer/download/getmodelsaddlist')}}",
        fnServerParams: function(aoData){
            aoData.push( { "name": "iDealerId", "value": {{$iDealerId}} } );
            aoData.push( { "name": "iCarBrandId", "value": {{$iCarBrandId}} } );
        },
        "ajax": "{{ url('web/product/car/dealer/download/getmodelsaddlist')}}",
    ...
    $('.btn-save').on('click', function(){
        ...
        var valuelist = '';
        var rows = table.rows({ 'search': 'applied' }).nodes();
        $('input:checkbox:checked[name="models"]', rows).each(function(){
            valuelist += $(this).val() + ",";
        });
        ...
        $.ajax({
            url : "{{ url('web/product/car/dealer/download/domodelsadd')}}",
            data : data,
            type : "POST",
