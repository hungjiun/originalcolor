...
class CustomerController extends _WebController
{
    ...
    public function index ()
    {
        $this->breadcrumb = [
            $this->module => "#",
            $this->module . "." . $this->action . ".customer" => url( 'web/' . $this->module . '/' . $this->action . '/customer' )
        ];

        $this->title = $this->module . "." . $this->action . ".customer";

        $this->func = "web." . $this->module . "." . $this->action . ".customer";
        $this->__initial();
    ...
    public function getList ()
    {
        ...
        } )->where( 'iAcType', 'like', '9%' )->select( 'sys_member.*', 'sys_member_info.*' )->get();
    ...
    public function add ()
    {
        $this->breadcrumb = [
            $this->module => "#",
            $this->module . "." . $this->action . ".customer.add" => url( 'web/' . $this->module . '/' . $this->action . '/customer/add' )
        ];

        $this->title = $this->module . "." . $this->action . ".customer.add";

        $this->func = "web." . $this->module . "." . $this->action . ".customer.add";
        $this->__initial();
     ...
     *
     */
    public function doMemberInfoSave ()
    {
        $id = ( Input::has( 'iId' ) ) ? Input::get( 'iId' ) : 0;
        if ( !$id) {
            $this->rtndata ['status'] = 0;
        ...
        if ($DaoMemberInfo->save()) {
            ...
            $this->_saveLogAction( $DaoMemberInfo->getTable(), $DaoMemberInfo->iId, 'edit', json_encode( $DaoMemberInfo ) );
            $this->rtndata ['status'] = 1;
            $this->rtndata ['message'] = trans( '_web_message.save_success' );
        } else {
            $this->rtndata ['status'] = 0;
