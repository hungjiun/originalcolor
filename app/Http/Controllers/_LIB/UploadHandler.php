...
namespace App\Http\Controllers\_LIB;
...
class UploadHandler
{
    ...
    function __construct ( $options = null, $initialize = true, $error_messages = null )
    {
        $this->response = [];
        $this->options = [
            'script_url' => $this->get_full_url() . '/fs1',
            'upload_dir' => env( 'UPLOAD_PATH', dirname( $this->get_server_var( 'SCRIPT_FILENAME' ) ) . '/' ) . config()->get( 'config.path.userdata' ) . session()->get( 'member.vUserCode' ) . '/' . date( 'Ymd' ) . '/',
            'upload_url' => $this->get_full_url() . '/' . config()->get( 'config.path.userdata' ) . session()->get( 'member.vUserCode' ) . '/' . date( 'Ymd' ) . '/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            ...
            'delete_type' => 'DELETE',
            'delete_url' => $this->get_full_url() . '/web/delete_image?',
    ...
    protected function set_additional_file_properties ( $file )
    {
        ...
        $file->deleteUrl = $this->options ['delete_url'] . $this->get_singular_param_name() . '=' . rawurlencode( $file->name );;
